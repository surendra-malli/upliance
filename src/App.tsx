...
const TopBar: React.FC = () => {
  ...
  return (
    <AppBar 
      ...
      color="default" 
      elevation={2} 
      sx={{ borderBottom: '1px solid rgba(15,17,26,0.06)' }}
    >
      {/* Main toolbar with logo */}
      <Toolbar sx={{ justifyContent: isMobile ? 'center' : 'flex-start', gap: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <AutoAwesomeIcon color="primary" sx={{ fontSize: 28 }} />
          <Typography variant="h6">Upliance.ai</Typography>
        </Box>
        
        {/* Desktop navigation - show buttons inline */}
        {!isMobile && (
          <>
            <Box sx={{ flex: 1 }} />
            <Button 
            ...
            <Button 
              component={Link} 
              to="/create" 
              color={loc.pathname.startsWith('/create') ? 'primary' : 'inherit'}
            >
              Create
            </Button>
            <Button 
              component={Link} 
              to="/myforms" 
              color={loc.pathname === '/myforms' ? 'primary' : 'inherit'}
            >
              My Forms
            </Button>
          </>
